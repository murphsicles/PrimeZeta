...
fn main() {
                ...
                match result {
                    Ok((remaining, _)) => {
                        if remaining.is_empty() {
                            ...
                            failed += 1;
                            println!("❌ {} - Partial parse ({} chars remaining)", path_str, remaining.len());
                            failures.push((path_str.to_string(), format!("Partial parse: {} chars remaining", remaining.len())));
                        }
                    }
                    Err(e) => {
                        failed += 1;
    ...
    if compatibility_percentage >= 50.0 {
        println!("✅ v0.5.0 compatibility: {:.1}% - MEETS v0.3.24 TARGET!", compatibility_percentage);
    } else {
        println!("❌ v0.5.0 compatibility: {:.1}% - DOES NOT MEET v0.3.24 TARGET", compatibility_percentage);
        std::process::exit(1);
    }
...
fn test_key_features() {
    let test_cases = vec![
        ...
        "#),
        ("Function with return type", r#"
        fn main() -> i64 {
            42
        }
        "#),
        ("Match expression (simplified)", r#"
        fn test(x: i64) -> i64 {
            x
        }
        "#),
