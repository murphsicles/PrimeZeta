...
use zetac::frontend::errors::{format_incomplete_parse, DiagnosticKind, Diagnostic};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut input = None;
    let mut output = None;
    let mut opt_level = zetac::middle::optimization::OptLevel::O0;
    let mut i = 1;
    while i < args.len() {
        match args[i].as_str() {
            "-o" => {
            ...
            "-O0" => {
                opt_level = zetac::middle::optimization::OptLevel::O0;
            }
            "-O1" => {
                opt_level = zetac::middle::optimization::OptLevel::O1;
            }
            "-O2" => {
                opt_level = zetac::middle::optimization::OptLevel::O2;
            }
            "-O3" => {
                opt_level = zetac::middle::optimization::OptLevel::O3;
            }
        ...
        match result {
            Ok((remaining, asts)) => {
                if !remaining.is_empty() {
                    eprint!("{}", format_incomplete_parse(&code, remaining, Some(&file)));
                    return Err("Incomplete parse".into());
                }
                ...
                if !type_ok {
                    let diag = Diagnostic {
                        ...
                        message: format!("type checking failed in {}", file),
                        loc: None,
                        source_line: None,
                        hint: Some("check function signatures, argument types, and return types".to_string()),
                    };
                    eprintln!("{}", diag);
                ...
                // Apply optimizations
                if opt_level != zetac::middle::optimization::OptLevel::O0 {
                    for mir in &mut all_mirs {
                        zetac::middle::optimization::optimize(mir, opt_level);
                    }
                }
            ...
            Err(e) => {
                // Try to extract remaining input from the nom error for location info
                let error_msg = zetac::frontend::errors::format_parse_error(&code, &e, Some(&file));
                eprintln!("{}", error_msg);
...
fn repl(_dump_mir: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        let asts = match parse_zeta(&code) {
            Ok((_, asts)) => asts,
            Err(e) => {
        ...
        match codegen.finalize_and_jit() {
            Ok(ee) => {
