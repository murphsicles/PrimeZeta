...
fn test_generic_struct_compilation() {
    ...
    if result.is_ok() {
        ...
        assert_eq!(result.unwrap(), 100, "Should return correct value");
    } else {
...
fn test_type_system_integration() {
    ...
    if result.is_ok() {
        ...
        assert_eq!(result.unwrap(), 10, "Should return max value");
    } else {
...
fn test_repeated_compilation() {
    ...
    for i in 0..3 {
        ...
        assert_eq!(result.unwrap(), 123, "Should return correct value each time");
    }
...
fn test_comprehensive_report() {
    ...
    let tests = vec![
        ("Basic compilation", test_basic_end_to_end_compilation as fn()),
        ("Generic structs", test_generic_struct_compilation as fn()),
        ("Generic functions", test_generic_function_compilation as fn()),
        ...
        ("Type system integration", test_type_system_integration as fn()),
    ...
    println!("\nOverall: {}/{} tests passed ({:.1}%)", passed, total, (passed as f32 / total as f32) * 100.0);
    ...
    assert!(true, "Report test should always pass");
}
...
fn test_lt_syntax_diagnostic() {
    ...
    let test_cases = vec![
        ("lt(Vec, i32)::new()", r#"fn main() -> i64 { let v = lt(Vec, i32)::new(); 0 }"#),
        ("lt(Option, i32)::Some(42)", r#"fn main() -> i64 { let o = lt(Option, i32)::Some(42); 0 }"#),
        ("lt(Result, i32, String)::Ok(42)", r#"fn main() -> i64 { let r = lt(Result, i32, String)::Ok(42); 0 }"#),
    ];
