...
fn test_v0_5_0_generic_function_parsing() {
    ...
    let result = parse_zeta(code);
    assert!(result.is_ok(), "Should parse generic function with multiple params");
...
fn test_v0_5_0_error_messages() {
    ...
    assert!(error_str.contains("Vec") || error_str.contains("i32") || error_str.contains("mismatch"));
}
...
fn test_v0_5_0_generic_performance() {
    ...
    for i in 0..10 {
        ...
        assert!(result.is_ok(), "Unification should succeed on iteration {}", i);
    }
    
    let duration = start.elapsed();
    
    // Should complete quickly (under 1 second)
    assert!(duration.as_secs() < 1, "Generic operations took too long: {:?}", duration);
