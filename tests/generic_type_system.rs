...
fn test_basic_generic_instantiation() {
    ...
    let instantiated = result_type.instantiate_generic(&[Type::I32, Type::Named("String".to_string(), vec![])]);
    assert!(instantiated.is_ok());
...
fn test_generic_function_unification() {
    ...
    let t_var2 = Type::Variable(TypeVar::fresh());
    let generic_func2 = Type::Function(
        vec![t_var2.clone()],
        Box::new(t_var2.clone())
    );
    
    let mismatched_func = Type::Function(
        vec![Type::I32],
        Box::new(Type::Bool)
    );
...
fn test_generic_with_trait_bounds() {
    ...
    let instantiated = container_type.instantiate_generic(&[Type::Named("String".to_string(), vec![])]);
    assert!(instantiated.is_ok());
...
fn test_nested_generics() {
    ...
    let instantiated = vec_type.instantiate_generic(&[option_i32.clone()]);
    assert!(instantiated.is_ok());
...
fn test_recursive_generic_types() {
    ...
    let boxed_list = Type::Ptr(Box::new(list_type.clone()));
...
fn test_generic_unification_variance() {
    ...
    let ok_type = Type::Named("Ok".to_string(), vec![Type::I32, Type::Named("String".to_string(), vec![])]);
    
    assert!(subst.unify(&result_type, &ok_type).is_ok());
    assert_eq!(subst.apply(&t_var2), Type::I32);
    assert_eq!(subst.apply(&e_var), Type::Named("String".to_string(), vec![]));
}
...
fn test_generic_type_display() {
    ...
    let nested = Type::Named("Vec".to_string(), vec![
        Type::Named("Option".to_string(), vec![Type::I32])
    ]);
...
fn test_type_variable_tracking() {
    ...
    let nested = Type::Named("HashMap".to_string(), vec![
        Type::Named("String".to_string(), vec![]),
    ...
    let concrete_nested = Type::Named("HashMap".to_string(), vec![
        Type::Named("String".to_string(), vec![]),
