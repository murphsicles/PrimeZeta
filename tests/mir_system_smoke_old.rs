...
fn test_mir_smoke_creation() {
    ...
    mir.stmts.push(MirStmt::Return { val: expr1_id });
...
fn test_mir_smoke_optimization_interface() {
    ...
    use zetac::middle::optimization::{constant_folding, dead_code_elimination};
    ...
    println!(
        "Optimizations completed on MIR with {} expressions and {} statements",
        mir.exprs.len(),
        mir.stmts.len()
    );
}
