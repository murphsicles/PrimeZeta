...
fn test_type_system_smoke_basic_types() {
    ...
    for (ty, expected_display) in types {
        ...
        assert_eq!(display, expected_display, "Type display mismatch for {:?}", ty);
    }
}
...
fn test_type_system_smoke_type_variables() {
    ...
    assert_ne!(format!("{:?}", var1), format!("{:?}", var2), "Type variables should be distinct");
    
    // Test creating variable type
    let ty_var = Type::Variable(var1);
    assert!(matches!(ty_var, Type::Variable(_)), "Should create variable type");
}
...
fn test_type_system_smoke_function_types() {
    ...
    assert!(display.contains("i32"), "Function type should include parameter types");
    assert!(display.contains("f64"), "Function type should include parameter types");
    assert!(display.contains("bool"), "Function type should include return type");
}
...
fn test_type_system_smoke_type_application() {
    ...
    assert_eq!(applied, Type::I32, "Variable should be substituted with i32");
}